...
for i in range(len(shift_usr)):
    ...
    if shift_usr[i].upper() == shift_usr[i]:
        flag = '1'
    else:
        flag = '2' # Если входящий символ имеет верхний регистр. Flag = True.

        if shift_usr[i] in rus_alf:
            int_med_res = rus_alf[eval(''.join(map(str, ('(', rus_alf.index(shift_usr[i].lower()), sign, shift_step, ')', '%', len(rus_alf)))))]
        elif shift_usr[i] in eng_alf:
            int_med_res = eng_alf[eval(''.join(map(str, ('(', eng_alf.index(shift_usr[i].lower()), sign, shift_step, ')', '%', len(eng_alf)))))]
    ...
    if flag in '1':
        res += int_med_res.upper()
    else:
        res += int_med_res # Если буква перед (де)шифровкой имела верхний регистр то после шифровки новую букву возводим в верхний регистр.
